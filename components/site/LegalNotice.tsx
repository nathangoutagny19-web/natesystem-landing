...
export default function LegalNotice({ lang }: { lang: Lang }) {
  const en = lang === 'en'
  const d = (fr: string, e: string) => (en ? e : fr)
  const home = en ? '/en' : '/'
  const back = d('← Retour au site', '← Back to the site')
  ...
  return (
    <div style={{ minHeight: '100vh' }}>
      <div className="mx-auto" style={{ maxWidth: '720px', padding: '80px 24px 60px', fontFamily: 'var(--font-sans)' }}>
        ...
        <h1 className="font-serif italic" style={{ fontSize: 'clamp(32px, 5vw, 44px)', fontWeight: 400, color: 'var(--text)', marginBottom: en ? '16px' : '40px', lineHeight: 1.15 }}>
        ...
        <Section title={d('Responsable du traitement', 'Data controller')}>
          <p><strong>Nathan Goutagny, NateSystem</strong></p>
          <p>{d('Entrepreneur Individuel', 'Sole trader (entrepreneur individuel)')}<br />SIRET : 92949816000035<br />{d('Lyon, France', 'Lyon, France')}</p>
          <p>{d('Contact', 'Contact')} : <a href="mailto:nathan@natesystem.com" style={{ color: 'var(--accent)', textDecoration: 'none' }}>nathan@natesystem.com</a></p>
        </Section>

        <Section title={d('Hébergement', 'Hosting')}>
          <p>{d(
        ...
        <Section title={d('Données collectées', 'Data collected')}>
          <p>{d(
          ...
          <ul style={{ paddingLeft: '20px', color: 'var(--text-secondary)', fontSize: '14px', lineHeight: 1.7 }}>
            <li>{d('Données de navigation anonymisées (cookies analytiques)', 'Anonymised browsing data (analytics cookies)')}</li>
            <li>{d('Informations fournies volontairement lors des prises de contact', 'Information you provide yourself when getting in touch')}</li>
            <li>{d('Données de réservation via Cal.com (service tiers)', 'Booking data through Cal.com (a third-party service)')}</li>
          </ul>
        </Section>

        <Section title={d('Politique de confidentialité', 'Privacy policy')}>
          <p>{d(
        ...
        <Section title={d('Cookies', 'Cookies')}>
          <p>{d(
        ...
        <Section title={d('Propriété intellectuelle', 'Intellectual property')}>
          <p>{d(
        ...
        <Section title={d('Limitation de responsabilité', 'Limitation of liability')}>
          <p>{d(

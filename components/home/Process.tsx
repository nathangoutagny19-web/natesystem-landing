    ...
    <section id="process" style={{ padding: '120px 24px' }}>
      <div className="mx-auto" style={{ maxWidth: '1100px' }}>
        <FadeUp className="text-center mb-20">
          {home ? (
            <>
              <span className="section-label">{d('Notre méthode', 'Our method', 'Módszerünk')}</span>
              <h2 className="section-title" style={{ maxWidth: '700px', margin: '0 auto 24px' }}>
                {d('Une méthode claire, ', 'A clear method, ', 'Világos módszer, ')}
        ...
        <FadeUp>
          <div className="hww-cta-wrap">
            <Link href={localizedHref('/methode', lang)} className="btn-ghost">
              {d('Voir comment on travaille en détail', 'See how we work in detail', 'Nézze meg részletesen, hogyan dolgozunk')} &rarr;
            </Link>
          </div>
        </FadeUp>
      </div>

...
export default function DownloadModal({ resource, onClose }: { resource: Resource | null; onClose: () => void }) {
  ...
  const inputClass = 'w-full px-4 py-3 rounded-xl text-sm focus:outline-none focus:ring-2 focus:ring-[#ED4245]/30 focus:border-[#ED4245] transition'
  ...
  return (
    ...
    >
      <div ref={modalRef} className="relative rounded-[20px] max-w-md w-full mx-auto p-8 shadow-2xl max-h-[90vh] overflow-y-auto" style={{ background: 'var(--bg-card)' }}>
        <button onClick={onClose} className="absolute top-4 right-4 transition-colors" style={{ color: 'var(--text-muted)' }} aria-label="Fermer">
        ...
        {status === 'success' ? (
          ...
          <>
            <h2 className="text-xl font-bold pr-8" style={{ color: 'var(--text)' }}>
            ...
            <form onSubmit={handleSubmit} className="mt-6 space-y-4">
              <div>
                ...
                <input type="text" value={form.prenom} onChange={e => updateField('prenom', e.target.value)} placeholder="Ton prénom" className={inputClass} style={{ border: '1px solid var(--border)', background: 'var(--bg-elevated)', color: 'var(--text)' }} required />
              </div>
              ...
              <div>
                ...
                <input type="email" value={form.email} onChange={e => updateField('email', e.target.value)} placeholder="prenom@entreprise.com" className={inputClass} style={{ border: '1px solid var(--border)', background: 'var(--bg-elevated)', color: 'var(--text)' }} required />
              </div>
              ...
              <div>
                ...
                <select value={form.role} onChange={e => updateField('role', e.target.value)} className={inputClass} style={{ border: '1px solid var(--border)', background: 'var(--bg-elevated)', color: 'var(--text)' }} required>
              ...
              <div>
                ...
                <select value={form.secteur} onChange={e => updateField('secteur', e.target.value)} className={inputClass} style={{ border: '1px solid var(--border)', background: 'var(--bg-elevated)', color: 'var(--text)' }} required>

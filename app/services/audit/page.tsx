...
type Step = { titleFr: string; titleEn: string; descFr: string; descEn: string; metricFr: string; metricEn: string }

const steps: Step[] = [
  {
    titleFr: 'On cartographie vos process',
    titleEn: 'We map your processes',
    ...
    descEn: 'How you actually work, laid flat: who does what, in what order, with what information. We spot what blocks, what slows down, what gets lost.',
    metricFr: 'Une vision claire de votre fonctionnement',
    metricEn: 'A clear view of how you run',
  },
  {
    titleFr: 'On chiffre le coût de l\u2019inaction',
    titleEn: 'We put a number on doing nothing',
    ...
    descEn: 'Hours lost, margin leaking away, know-how sitting in people\u2019s heads. We put a figure on the value you leave on the table, not only on what you spend.',
    metricFr: 'Le coût réel, en euros',
    metricEn: 'The real cost, in euros',
  },
  {
    titleFr: 'On dessine la cible',
    titleEn: 'We draw the target',
    ...
    descEn: 'Your operations as they would run with custom software, and AI where it genuinely helps. Concrete, not theoretical.',
    metricFr: 'Le « après », à l\u2019échelle de votre métier',
    metricEn: 'The “after”, at the scale of your trade',
  },
  {
    titleFr: 'On vous remet le plan',
    titleEn: 'We hand you the plan',
    ...
    descEn: 'A clear, costed action plan, ordered by impact. You can run it with us, in house, or both. It is yours to keep, with or without us.',
    metricFr: 'Un plan exécutable, livré',
    metricEn: 'A plan you can act on, delivered',
  },
]
    ...
    <main>
      ...
      <section style={{ padding: '160px 24px 40px' }}>
        <div className="mx-auto text-center" style={{ maxWidth: 820 }}>
          <FadeUp>
            <span className="section-label">{d('Audit & consulting', 'Audit & consulting')}</span>
            <h1 className="font-serif italic" style={{ fontSize: 'clamp(32px, 5.4vw, 54px)', fontWeight: 400, lineHeight: 1.1, color: 'var(--text)', maxWidth: 780, margin: '14px auto 24px' }}>
              {d('On comprend votre activité ', 'We understand your business ')}<span className="accent" style={{ color: 'var(--accent)' }}>{d('avant de construire quoi que ce soit.', 'before we build anything at all.')}</span>
            </h1>
        ...
        <div className="mx-auto" style={{ maxWidth: 760 }}>
          <FadeUp className="text-center mb-8">
            <h2 className="section-title" style={{ maxWidth: 640, margin: '0 auto' }}>
              {d('En quoi consiste ', 'What does ')}<span className="accent">{d('un audit de process', 'a process audit')}</span>{d(' ?', ' involve?')}
            </h2>
          </FadeUp>
          <FadeUp>
      ...
      <section style={{ padding: '70px 24px' }}>
        <div className="mx-auto" style={{ maxWidth: 760 }}>
          <FadeUp className="text-center mb-10">
            <span className="section-label">{d('Si vous vous reconnaissez', 'If this sounds like you')}</span>
            <h2 className="section-title" style={{ maxWidth: 620, margin: '0 auto' }}>
              {d('L\u2019audit est ', 'The audit is ')}<span className="accent">{d('pour vous.', 'for you.')}</span>
            </h2>
          </FadeUp>
      ...
      <section style={{ padding: '70px 24px' }}>
        <div className="mx-auto" style={{ maxWidth: 1100 }}>
          <FadeUp className="text-center mb-12">
            <span className="section-label">{d('Comment se passe l\u2019audit', 'How the audit runs')}</span>
            <h2 className="section-title" style={{ maxWidth: 660, margin: '0 auto' }}>
              {d('Quatre temps, ', 'Four stages, ')}<span className="accent">{d('un plan clair.', 'one clear plan.')}</span>
            </h2>
          </FadeUp>
          ...
          <FadeUp>
            <div style={{ background: 'var(--bg-card)', border: '1px solid rgba(230,57,70,0.15)', borderRadius: 12, padding: '48px 40px', textAlign: 'center' }}>
              <h2 className="font-serif italic" style={{ fontSize: 'clamp(24px, 4vw, 32px)', fontWeight: 400, marginBottom: 16, color: 'var(--text)' }}>
                {d('Sachez où agir en premier.', 'Know where to act first.')}
              </h2>
